    ...
    # 开始训练
        ...
        for self.c_puct in self.c_puct_list:
            for self.n_job_thread in self.n_job_thread_list:
                self.path = (r'D:\科研\论文\High effient resource scheduling for cloud based on modified MCTS\programing\parameter_check_on_') + self.data_name + '_cpuct_'+ str(self.c_puct) + '_njobthread_' +str(self.n_job_thread) +'.pkl'
